...
This document provides practical guidance for running, monitoring, and troubleshooting the Network Intrusion Detection System (NIDS) in day-to-day operations.

---

## Operating Modes

The main entry point is `main.py`, which supports multiple modes: detection, training, and API.

### Real-Time Detection Mode

Run real-time detection on a network interface (requires elevated privileges):
...
- Sends alerts via configured channels (email, Slack, Syslog) and optionally logs to PostgreSQL.

### Model Training Mode

Train or retrain ML models on the configured dataset:
...
- Save updated model artifacts under `models/` (e.g., `nids_model.pkl`, `scaler.pkl`).
...
- Calling REST API endpoints as described in `docs/API_DOCUMENTATION.md`.

---

## Configuration Management

Main configuration file: `config/nids_config.yaml` (and optional overrides like `config/nids_config.local.yaml`).

Typical configuration sections:

- **Network**
  - Interfaces to monitor (e.g., `eth0`, `wlan0`).
  - Packet filter expressions (e.g., BPF-style filters for ports/protocols).

- **Detection**
  - Model path (e.g., `models/nids_model.pkl`).
  - Thresholds for anomaly scores.
  - Enabled algorithms (Random Forest, Isolation Forest, etc.).

- **Alerts**
  - Enable/disable alerting.
  - Email server and from-address.
  - Slack webhook URL, Syslog targets, and other notifier settings.

- **Logging**
  - Log level and output (console, file).
  - Database connection (PostgreSQL DSN) if audit logging is enabled.
...
- Detection reason (e.g., rule ID, anomaly score above threshold).
...
- **Database connectivity**
  - Confirm logs are being written to PostgreSQL when enabled.

If Prometheus metrics are enabled (planned/optional):

- Monitor throughput, detection latency, and error rates via your preferred metrics stack.
...
- Review packet filter expressions in `config/nids_config.yaml` to ensure they are not too restrictive.
...
- Check logs for errors in the detection pipeline or model loading.
...
- Review features and model assumptions; retrain models with more representative data if needed.
...
- Inspect database logs for authentication or permission errors.
...
1. **Backup configuration and models**
   - Save copies of `config/*` and `models/*`.
...
Keep a simple changelog (or rely on Git tags and release notes) so operators know which version is deployed and what changed.

---

For detailed architecture and threat scenarios, see `docs/ARCHITECTURE.md` and `docs/THREAT_MODELS.md`.
